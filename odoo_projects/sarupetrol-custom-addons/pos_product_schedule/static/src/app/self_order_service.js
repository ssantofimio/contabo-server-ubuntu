...
patch(SelfOrder.prototype, {
    ...
    async setup(env, { rpc, notification, router, printer, barcode }) {
        ...
        try {
            // In v18, data is often in this.models or this.data
            const schedules = this.data.models['pos.product.schedule']?.getAll() || [];
            ...
            this.config.tz = this.config.tz || "UTC";
        } catch (e) {
            // ignore
        }
    },
    ...
    initData() {
        ...
        // In Odoo 18, we can't easily filter the global model collection without side effects, 
        // but we can override the grouping logic used by the UI.

        const allProducts = this.models['product.product'].getAll();
        const availableProducts = allProducts.filter(p => this.shouldAddProduct(p));

        // Rebuild grouped products after filtering (v18 uses productByCategIds)
        this.productByCategIds = availableProducts.reduce((acc, product) => {
            product.pos_categ_ids.map((categ) => {
                const categId = categ.id || categ;
                acc[categId] = acc[categId] || [];
                acc[categId].push(product);
            });
            return acc;
        }, {});
    },
