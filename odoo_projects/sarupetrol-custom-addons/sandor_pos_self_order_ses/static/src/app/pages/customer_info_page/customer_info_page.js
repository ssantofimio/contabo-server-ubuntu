...
export class CustomerInfoPage extends Component {
        ...
        onWillStart(async () => {
            try {
                ...
                if (!this.selfOrder.countries || !this.selfOrder.states || !this.selfOrder.identification_types) {
                    const data = await rpc("/pos-self-order/get-customer-form-data", {
    ...
    async searchPartner() {
        ...
        try {
            const partner = await rpc("/pos-self-order/search-partner-by-nif", {
                nif: this.state.vat,
                access_token: this.selfOrder.access_token,
            });
            ...
            if (partner) {
                ...
                if (partner.active_orders && partner.active_orders.length > 0) {
                    this.selfOrder.models.loadData({ 'pos.order': partner.active_orders });
                    ...
                    if (this.selfOrder.table && this.selfOrder.table.is_busy) {
                        targetOrder = this.selfOrder.models['pos.order'].find(o =>
                    ...
                    if (!targetOrder) {
                        targetOrder = this.selfOrder.models['pos.order'].filter(o => o && o.partner_id && o.state !== 'paid' && o.state !== 'cancel')
    ...
    async confirmAndPay() {
        ...
        try {
            if (!partnerId) {
                const response = await rpc("/pos-self-order/create-partner-custom", {
                ...
                const response = await rpc("/pos-self-order/update-partner-custom", {
