...
class OutOfStockReport(models.AbstractModel):
    ...
    _name = 'report.custom_reports.report_inventory_out_of_stock'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        total_out_of_stock_qty = sum(item.get('out_of_stock_qty', 0) for item in result_data)
        
        # Batch fetch product costs
        product_ids = [data.get('product_id') for data in result_data]
        products = self.env['product.product'].browse(product_ids)
        product_map = {p.id: p for p in products}

        for data in result_data:
            product_id = data.get('product_id')
            out_of_stock_qty = data.get('out_of_stock_qty', 0)
            
            data['out_of_stock_qty_percentage'] = round((out_of_stock_qty / total_out_of_stock_qty * 100), 2) if total_out_of_stock_qty else 0.0
