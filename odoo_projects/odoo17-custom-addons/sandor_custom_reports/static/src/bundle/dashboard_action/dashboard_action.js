...
export class SpreadsheetDashboardAction extends Component {
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (this.props.state && this.props.state.dashboardLoader) {
                ...
                if (params.config_id) {
                    await this.loader.loadFromConfig(params.config_id);
                } else {
    ...
    async shareSpreadsheet(data, excelExport) {
        const url = await this.orm.call("sandor.spreadsheet.dashboard.share", "action_get_share_url", [
            {
...
SpreadsheetDashboardAction.template = "sandor_custom_reports.DashboardAction";
...
registry
    .category("actions")
    .add("action_sandor_custom_reports", SpreadsheetDashboardAction, { force: true });
