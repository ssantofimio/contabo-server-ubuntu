...
class InventoryFsnXyzReport(models.TransientModel):
    ...
    start_date = fields.Date('Fecha de Inicio',
                             help="Fecha de inicio para analizar el reporte",
                             required=True)
    end_date = fields.Date('Fecha de Fin',
                           help="Fecha de fin para analizar el reporte",
                           required=True)
    warehouse_ids = fields.Many2many(
        "stock.warehouse", string="Almacenes",
        help="Seleccione los almacenes para generar el reporte")
    ...
    def action_pdf(self):
        ...
        return (
            self.env.ref(
                'custom_reports.report_inventory_fsn_xyz_action')
    ...
    def get_xlsx_report(self, data, response):
        ...
        if start_date and end_date:
            sheet.write('A5',  'Fecha de Inicio: ', bold_format)
            sheet.write('B5', start_date, txt)
            sheet.write('A6',  'Fecha de Fin: ', bold_format)
            sheet.write('B6', end_date, txt)
        headers = ['Producto', 'Categoría', 'Stock Inicial', 'Stock Final',
                   'Stock Promedio', 'Ventas', 'Rotación', 'Stock Actual',
                   'Valor del Stock', 'Stock Value(%)', 'Cumulative Value(%)',
                   'Clasificación FSN', 'Clasificación XYZ',
